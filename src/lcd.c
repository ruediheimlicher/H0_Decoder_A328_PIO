...
*/
extern void lcd_load_byte(uint8_t out_byte);
...
unsigned char dat[4];
  ...
  dat[3]='\0';                       // String Terminator
  for(i=2; i>=0; i--) 
  {
    dat[i]=(zahl % 10) +'0';         // Modulo rechnen, dann den ASCII-Code von '0' addieren
    zahl /= 10;
  }
lcd_puts(dat);
}
...
void r_uitoa16(uint32_t zahl, unsigned char* string) 
{
  int8_t i;                             // schleifenz�hler
...
void r_itoa(int32_t zahl, unsigned char* dat) 
{
  uint8_t i;
 
  dat[11]='\0';                  // String Terminator
  if( zahl < 0 ) {                  // ist die Zahl negativ?
    dat[0] = '-';              
    zahl = -zahl;
  }
  else dat[0] = ' ';             // Zahl ist positiv
...
void r_itoa16(int16_t zahl, unsigned char* dat) 
{
  uint8_t i;
 
  dat[7]='\0';                  // String Terminator
  if( zahl < 0 ) {                  // ist die Zahl negativ?
    dat[0] = '-';              
    zahl = -zahl;
  }
...
void lcd_put_frac(unsigned char* dat, uint8_t start, uint8_t komma, uint8_t frac) 
{
  ...
  uint8_t i;            // Zaehler
  uint8_t flag=0;       // Merker for fuehrende Nullen
 
  // Vorzeichen ausgeben  
  if (dat[0]=='-') lcd_putc('-'); else lcd_putc(' ');
  ...
  for(; i<(komma+frac); i++) lcd_putc(dat[i]);
...
{
	 unsigned char* wochentag[] = {(unsigned char*)"MO",(unsigned char*)"DI",(unsigned char*)"MI",(unsigned char*)"DO",(unsigned char*)"FR",(unsigned char*)"SA",(unsigned char*)"SO"};
...
/*
	...
	*/	
		lcd_puts((unsigned char*)outstring);
